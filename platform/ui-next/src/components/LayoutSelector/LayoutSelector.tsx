  ...
  const button = (
    <Button
      className={cn(
        ...
        disabled
          ? 'text-common-bright hover:bg-[#424242] hover:text-primary-light cursor-not-allowed opacity-40'
          : isOpen
  ...
  return (
    ...
    >
      {Array.from(Array(rows * columns).keys()).map(index => (
        <div
          key={index}
          className={cn('cursor-pointer', isHovered(index) ? 'bg-primary-active' : 'bg-[#2a2a2a]')}
